...
export async function LandingHomepage({
  appName,
  session
}: LandingHomepageProps) {
  const locale = await getLocale();
  const t = await getTranslations('Landing');
  const createAccountHref = session
    ? landingPageLinks.trade
    : landingPageLinks.signUp;
    ...
    >
      ...
      <section className='relative flex min-h-[90vh] items-center overflow-hidden bg-[#181818] pt-20'>
        <div className='absolute inset-0 z-0'>
          ...
          <Image
            src='/images/hero/hp-hero-mobile.jpg'
            alt='Trading interface shown on a phone screen'
            fill
            className='object-cover opacity-50 grayscale md:hidden'
            priority
            sizes='100vw'
          />
          <Image
            src='/images/hero/hp-hero.jpg'
            alt='Professional stock market chart on a dark trading interface'
            fill
            className='hidden object-cover opacity-50 grayscale md:block'
            priority
            sizes='100vw'
          />
        </div>
          ...
          <div className='grid grid-cols-1 gap-16 md:grid-cols-2 md:gap-12'>
            <div className='space-y-6 border border-[#c4c7c7]/20 bg-[#F9F9F9] p-8 md:p-10'>
              <div className='flex items-center gap-4'>
                ...
                <h3
                  className={`${ebGaramond.className} text-2xl leading-8 font-medium`}
                >
                  {t('beginners.title')}
                </h3>
              </div>
            ...
            >
              <div className={`overflow-hidden ${card.aspect}`}>
                ...
                <div className='absolute bottom-8 left-8'>
                  ...
                  <h3
                    className={`${ebGaramond.className} text-2xl leading-8 font-medium text-white`}
                  >
